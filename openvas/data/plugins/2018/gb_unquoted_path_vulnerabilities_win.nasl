...
if( FEED_NAME == "GSF" || FEED_NAME == "GEF" || FEED_NAME == "SCM" ) {
  ...
  foreach service ( split( service_list ) ) {
    ...
    if ( "c:\windows\System32" >!< path_name && ' ' >< path_name ) {
  ...
  ps_cmd = "Get-CimInstance -query 'SELECT DisplayName, Name, PathName FROM Win32_Service WHERE NOT PathName LIKE ''%c:\\windows\\System32%'' AND PathName LIKE ''% %''' | ForEach-Object { $_.DisplayName + '|' + $_.Name + '|' + $_.PathName }";
  ...
  if( services ) {
    ...
    foreach service( services_list ) {
      ...
      if( service == "DisplayName|Name|PathName" )
        continue; # nb: Just ignoring the header, make sure to update this if you add additional fields to the CIM query above
