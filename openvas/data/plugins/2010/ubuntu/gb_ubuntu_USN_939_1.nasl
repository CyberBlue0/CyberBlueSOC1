...
if(description)
{
  ...
  script_name("Ubuntu: Security Advisory (USN-939-1)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2010 Greenbone AG");
  script_family("Ubuntu Local Security Checks");

  script_xref(name:"Advisory-ID", value:"USN-939-1");
  script_xref(name:"URL", value:"https://ubuntu.com/security/notices/USN-939-1");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'xorg-server' package(s) announced via the USN-939-1 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Loic Minier discovered that xvfb-run did not correctly keep the
...
privileges. (CVE-2010-1166)");
  ...
  script_tag(name:"affected", value:"'xorg-server' package(s) on Ubuntu 8.04, Ubuntu 9.04, Ubuntu 9.10.");
