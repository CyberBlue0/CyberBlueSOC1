...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-2045)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2010 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-2045");
  script_xref(name:"URL", value:"https://www.debian.org/security/2010/dsa-2045");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'libtheora' package(s) announced via the DSA-2045 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Bob Clary, Dan Kaminsky and David Keeler discovered that in libtheora, a video library part of the Ogg project, several flaws allow context-dependent attackers via a large and specially crafted media file, to cause a denial of service (crash of the player using this library), and possibly arbitrary code execution.
...
We recommend that you upgrade your libtheora packages.");
  ...
  script_tag(name:"affected", value:"'libtheora' package(s) on Debian 5.");
