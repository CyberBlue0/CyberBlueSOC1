...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.67635");
  ...
  script_name("Debian: Security Advisory (DSA-2063)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2010 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-2063");
  script_xref(name:"URL", value:"https://www.debian.org/security/2010/dsa-2063");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'pmount' package(s) announced via the DSA-2063 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Dan Rosenberg discovered that pmount, a wrapper around the standard mount program which permits normal users to mount removable devices without a matching /etc/fstab entry, creates files in /var/lock insecurely. A local attacker could overwrite arbitrary files utilising a symlink attack.
...
We recommend that you upgrade your pmount package.");
  ...
  script_tag(name:"affected", value:"'pmount' package(s) on Debian 5.");
