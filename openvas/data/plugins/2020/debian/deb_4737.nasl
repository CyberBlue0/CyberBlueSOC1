...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-4737)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2020 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-4737");
  script_xref(name:"URL", value:"https://www.debian.org/security/2020/dsa-4737");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/xrdp");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'xrdp' package(s) announced via the DSA-4737 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Ashley Newson discovered that the XRDP sessions manager was susceptible to denial of service. A local attacker can further take advantage of this flaw to impersonate the XRDP sessions manager and capture any user credentials that are submitted to XRDP, approve or reject arbitrary login credentials or to hijack existing sessions for xorgxrdp sessions.
