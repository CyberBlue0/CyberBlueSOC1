...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-4632)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2020 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-4632");
  script_xref(name:"URL", value:"https://www.debian.org/security/2020/dsa-4632");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/ppp");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'ppp' package(s) announced via the DSA-4632 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Ilja Van Sprundel reported a logic flaw in the Extensible Authentication Protocol (EAP) packet parser in the Point-to-Point Protocol Daemon (pppd). An unauthenticated attacker can take advantage of this flaw to trigger a stack-based buffer overflow, leading to denial of service (pppd daemon crash).
...
For the detailed security status of ppp please refer to its security tracker page at: [link moved to references]");

  script_tag(name:"affected", value:"'ppp' package(s) on Debian 9, Debian 10.");
