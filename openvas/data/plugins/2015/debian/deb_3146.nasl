...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-3146)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2015 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-3146");
  script_xref(name:"URL", value:"https://www.debian.org/security/2015/dsa-3146");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'requests' package(s) announced via the DSA-3146 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Jakub Wilk discovered that in requests, an HTTP library for the Python language, authentication information was improperly handled when a redirect occurred. This would allow remote servers to obtain two different types of sensitive information: proxy passwords from the Proxy-Authorization header ( CVE-2014-1830), or netrc passwords from the Authorization header ( CVE-2014-1829).
...
We recommend that you upgrade your requests packages.");
  ...
  script_tag(name:"affected", value:"'requests' package(s) on Debian 7.");
