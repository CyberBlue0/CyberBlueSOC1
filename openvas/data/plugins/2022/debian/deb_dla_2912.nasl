...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DLA-2912)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2022 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DLA-2912");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2022/dla-2912");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/libphp-adodb");
  script_xref(name:"URL", value:"https://wiki.debian.org/LTS");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'libphp-adodb' package(s) announced via the DLA-2912 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"It was found that in libphp-adodb, a PHP database abstraction layer library, an attacker can inject values into the PostgreSQL connection string by bypassing adodb_addslashes(). The function can be bypassed in phppgadmin, for example, by surrounding the username in quotes and submitting with other parameters injected in between.

For Debian 9 stretch, this problem has been fixed in version 5.20.9-1+deb9u1.
...
Further information about Debian LTS security advisories, how to apply these updates to your system and frequently asked questions can be found at: [link moved to references]");
  ...
  script_tag(name:"affected", value:"'libphp-adodb' package(s) on Debian 9.");
