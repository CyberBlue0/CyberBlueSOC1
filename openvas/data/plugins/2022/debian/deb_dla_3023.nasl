...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DLA-3023)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2022 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DLA-3023");
  script_xref(name:"URL", value:"https://www.debian.org/lts/security/2022/dla-3023");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/puma");
  script_xref(name:"URL", value:"https://wiki.debian.org/LTS");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'puma' package(s) announced via the DLA-3023 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Several security vulnerabilities have been discovered in puma, a web server for Ruby/Rack applications. These flaws may lead to information leakage due to not always closing response bodies, allowing untrusted input in a response header (HTTP Response Splitting) and thus potentially facilitating several other attacks like cross-site scripting. A poorly-behaved client could also use keepalive requests to monopolize Puma's reactor and create a denial of service attack.
