...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-2248)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2011 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-2248");
  script_xref(name:"URL", value:"https://www.debian.org/security/2011/dsa-2248");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'ejabberd' package(s) announced via the DSA-2248 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Wouter Coekaerts discovered that ejabberd, a distributed XMPP/Jabber server written in Erlang, is vulnerable to the so-called billion laughs attack because it does not prevent entity expansion on received data. This allows an attacker to perform denial of service attacks against the service by sending specially crafted XML data to it.
...
We recommend that you upgrade your ejabberd packages.");
  ...
  script_tag(name:"affected", value:"'ejabberd' package(s) on Debian 5, Debian 6.");
