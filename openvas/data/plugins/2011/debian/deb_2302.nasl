...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-2302)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2011 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-2302");
  script_xref(name:"URL", value:"https://www.debian.org/security/2011/dsa-2302");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'bcfg2' package(s) announced via the DSA-2302 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"It has been discovered that the Bcfg2 server, a configuration management server for Bcfg2 clients, is not properly sanitizing input from Bcfg2 clients before passing it to various shell commands. This enables an attacker in control of a Bcfg2 client to execute arbitrary commands on the server with root privileges.
...
We recommend that you upgrade your bcfg2 packages.");
  ...
  script_tag(name:"affected", value:"'bcfg2' package(s) on Debian 5, Debian 6.");
