...
if(description)
{
  ...
  script_tag(name:"creation_date", value:"2016-08-04 10:57:35 +0000 (Thu, 04 Aug 2016)");
  ...
  script_name("Debian: Security Advisory (DSA-3636)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2016 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-3636");
  script_xref(name:"URL", value:"https://www.debian.org/security/2016/dsa-3636");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'collectd' package(s) announced via the DSA-3636 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Emilien Gaspar discovered that collectd, a statistics collection and monitoring daemon, incorrectly processed incoming network packets. This resulted in a heap overflow, allowing a remote attacker to either cause a DoS via application crash, or potentially execute arbitrary code.
...
We recommend that you upgrade your collectd packages.");
  ...
  script_tag(name:"affected", value:"'collectd' package(s) on Debian 8.");
