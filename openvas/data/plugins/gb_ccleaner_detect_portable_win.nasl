...
if(description)
{
  ...
  script_dependencies("gb_ccleaner_detect_win.nasl", "lsc_options.nasl");
...
foreach filePath( keys( fileList ) ) {
  ...
  if( vers && version = eregmatch( string:vers, pattern:"^([0-9]+\.[0-9]+\.[0-9]+)" ) ) {
    ...
    register_and_report_cpe( app:"CCleaner Portable", ver:version[1], concluded:vers, base:cpe, expr:"^([0-9.]+)", insloc:location, regPort:0, regService:"smb-login" );
  }
}
