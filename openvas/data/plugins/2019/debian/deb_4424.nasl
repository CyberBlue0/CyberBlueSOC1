...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.704424");
  script_cve_id("CVE-2019-3871");
  script_tag(name:"creation_date", value:"2019-04-06 02:00:07 +0000 (Sat, 06 Apr 2019)");
  ...
  script_name("Debian: Security Advisory (DSA-4424)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2019 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-4424");
  script_xref(name:"URL", value:"https://www.debian.org/security/2019/dsa-4424");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/pdns");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'pdns' package(s) announced via the DSA-4424 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Adam Dobrawy, Frederico Silva and Gregory Brzeski from HyperOne.com discovered that pdns, an authoritative DNS server, did not properly validate user-supplied data when building a HTTP request from a DNS query in the HTTP Connector of the Remote backend. This would allow a remote user to cause either a denial-of-service, or information disclosure.
