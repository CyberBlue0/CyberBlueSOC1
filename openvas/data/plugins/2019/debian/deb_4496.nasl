...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-4496)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2019 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-4496");
  script_xref(name:"URL", value:"https://www.debian.org/security/2019/dsa-4496");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/pango1.0");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'pango1.0' package(s) announced via the DSA-4496 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Benno Fuenfstueck discovered that Pango, a library for layout and rendering of text with an emphasis on internationalization, is prone to a heap-based buffer overflow flaw in the pango_log2vis_get_embedding_levels function. An attacker can take advantage of this flaw for denial of service or potentially the execution of arbitrary code.
...
For the detailed security status of pango1.0 please refer to its security tracker page at: [link moved to references]");

  script_tag(name:"affected", value:"'pango1.0' package(s) on Debian 10.");
