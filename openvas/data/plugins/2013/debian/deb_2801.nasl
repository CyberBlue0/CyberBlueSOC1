...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-2801)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2013 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-2801");
  script_xref(name:"URL", value:"https://www.debian.org/security/2013/dsa-2801");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'libhttp-body-perl' package(s) announced via the DSA-2801 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Jonathan Dolle reported a design error in HTTP::Body, a Perl module for processing data from HTTP POST requests. The HTTP body multipart parser creates temporary files which preserve the suffix of the uploaded file. An attacker able to upload files to a service that uses HTTP::Body::Multipart could potentially execute commands on the server if these temporary filenames are used in subsequent commands without further checks.
...
We recommend that you upgrade your libhttp-body-perl packages.");
  ...
  script_tag(name:"affected", value:"'libhttp-body-perl' package(s) on Debian 7.");
