...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.66057");
  ...
  script_name("Debian: Security Advisory (DSA-1909)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2009 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1909");
  script_xref(name:"URL", value:"https://www.debian.org/security/2009/dsa-1909");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'postgresql-ocaml' package(s) announced via the DSA-1909 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"It was discovered that postgresql-ocaml, OCaml bindings to PostgreSQL's libpq, was missing a function to call PQescapeStringConn(). This is needed, because PQescapeStringConn() honours the charset of the connection and prevents insufficient escaping, when certain multibyte character encodings are used. The added function is called escape_string_conn() and takes the established database connection as a first argument. The old escape_string() was kept for backwards compatibility.
...
We recommend that you upgrade your postgresql-ocaml packages.");
  ...
  script_tag(name:"affected", value:"'postgresql-ocaml' package(s) on Debian 4, Debian 5.");
