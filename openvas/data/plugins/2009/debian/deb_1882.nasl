...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-1882)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2009 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1882");
  script_xref(name:"URL", value:"https://www.debian.org/security/2009/dsa-1882");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'xapian-omega' package(s) announced via the DSA-1882 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"It was discovered that xapian-omega, a CGI interface for searching xapian databases, is not properly escaping user supplied input when printing exceptions. An attacker can use this to conduct cross-site scripting attacks via crafted search queries resulting in an exception and steal potentially sensitive data from web applications running on the same domain or embedding the search engine into a website.
...
We recommend that you upgrade your xapian-omega packages.");
  ...
  script_tag(name:"affected", value:"'xapian-omega' package(s) on Debian 4, Debian 5.");
