...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.66055");
  ...
  script_name("Debian: Security Advisory (DSA-1911)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2009 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1911");
  script_xref(name:"URL", value:"https://www.debian.org/security/2009/dsa-1911");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'pygresql' package(s) announced via the DSA-1911 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"It was discovered that pygresql, a PostgreSQL module for Python, was missing a function to call PQescapeStringConn(). This is needed, because PQescapeStringConn() honours the charset of the connection and prevents insufficient escaping, when certain multibyte character encodings are used. The new function is called pg_escape_string(), which takes the database connection as a first argument. The old function escape_string() has been preserved as well for backwards compatibility.
...
We recommend that you upgrade your pygresql packages.");
  ...
  script_tag(name:"affected", value:"'pygresql' package(s) on Debian 4, Debian 5.");
