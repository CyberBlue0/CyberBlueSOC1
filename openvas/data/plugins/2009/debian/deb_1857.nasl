...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.64633");
  ...
  script_name("Debian: Security Advisory (DSA-1857)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2009 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1857");
  script_xref(name:"URL", value:"https://www.debian.org/security/2009/dsa-1857");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'camlimages' package(s) announced via the DSA-1857 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Tielei Wang discovered that CamlImages, an open source image processing library, suffers from several integer overflows which may lead to a potentially exploitable heap overflow and result in arbitrary code execution. This advisory addresses issues with the reading of JPEG and GIF Images, while DSA 1832-1 addressed the issue with PNG images.
