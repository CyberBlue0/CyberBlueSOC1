...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.66304");
  ...
  script_name("Ubuntu: Security Advisory (USN-853-1)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2009 Greenbone AG");
  script_family("Ubuntu Local Security Checks");

  script_xref(name:"Advisory-ID", value:"USN-853-1");
  script_xref(name:"URL", value:"https://ubuntu.com/security/notices/USN-853-1");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'firefox-3.0, firefox-3.5, xulrunner-1.9, xulrunner-1.9.1' package(s) announced via the USN-853-1 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Alin Rad Pop discovered a heap-based buffer overflow in Firefox when it
...
Brosnan, ... [Please see the references for more information on the vulnerabilities]");

  script_tag(name:"affected", value:"'firefox-3.0, firefox-3.5, xulrunner-1.9, xulrunner-1.9.1' package(s) on Ubuntu 8.04, Ubuntu 8.10, Ubuntu 9.04, Ubuntu 9.10.");
