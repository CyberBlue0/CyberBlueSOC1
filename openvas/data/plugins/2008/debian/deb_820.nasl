...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.55421");
  script_cve_id("CVE-2005-2769", "CVE-2005-2820");
  ...
  script_name("Debian: Security Advisory (DSA-820)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-820");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/dsa-820");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'courier' package(s) announced via the DSA-820 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Jakob Balle discovered that with 'Conditional Comments' in Internet Explorer it is possible to hide javascript code in comments that will be executed when the browser views a malicious email via sqwebmail. Successful exploitation requires that the user is using Internet Explorer.
