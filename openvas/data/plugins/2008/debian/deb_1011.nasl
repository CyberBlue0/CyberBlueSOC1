...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.56463");
  ...
  script_name("Debian: Security Advisory (DSA-1011)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1011");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/dsa-1011");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'kernel-patch-vserver, util-vserver' package(s) announced via the DSA-1011 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Several vulnerabilities have been discovered in the Debian vserver support for Linux. The Common Vulnerabilities and Exposures project identifies the following problems:

CVE-2005-4347

Bjorn Steinbrink discovered that the chroot barrier is not set correctly with util-vserver which may result in unauthorised escapes from a vserver to the host system.
...
We recommend that you upgrade your util-vserver and kernel-patch-vserver packages and build a new kernel immediately.");
  ...
  script_tag(name:"affected", value:"'kernel-patch-vserver, util-vserver' package(s) on Debian 3.1.");
