...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.56278");
  ...
  script_name("Debian: Security Advisory (DSA-975)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-975");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/dsa-975");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'nfs-user-server' package(s) announced via the DSA-975 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Marcus Meissner discovered that attackers can trigger a buffer overflow in the path handling code by creating or abusing existing symlinks, which may lead to the execution of arbitrary code.
...
We recommend that you upgrade your nfs-user-server package.");
  ...
  script_tag(name:"affected", value:"'nfs-user-server' package(s) on Debian 3.0, Debian 3.1.");
