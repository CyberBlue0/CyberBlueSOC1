...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53159");
  ...
  script_name("Debian: Security Advisory (DSA-461)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-461");
  script_xref(name:"URL", value:"https://www.debian.org/security/2004/dsa-461");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'calife' package(s) announced via the DSA-461 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Leon Juranic discovered a buffer overflow related to the getpass(3) library function in calife, a program which provides super user privileges to specific users. A local attacker could potentially exploit this vulnerability, given knowledge of a local user's password and the presence of at least one entry in /etc/calife.auth, to execute arbitrary code with root privileges.
...
We recommend that you update your calife package.");
  ...
  script_tag(name:"affected", value:"'calife' package(s) on Debian 3.0.");
