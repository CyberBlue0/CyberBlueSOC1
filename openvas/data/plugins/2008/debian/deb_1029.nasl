...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.56535");
  ...
  script_name("Debian: Security Advisory (DSA-1029)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1029");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/dsa-1029");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'libphp-adodb' package(s) announced via the DSA-1029 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Several vulnerabilities have been discovered in libphp-adodb, the 'adodb' database abstraction layer for PHP. The Common Vulnerabilities and Exposures project identifies the following problems:
...
We recommend that you upgrade your libphp-adodb package.");
  ...
  script_tag(name:"affected", value:"'libphp-adodb' package(s) on Debian 3.0, Debian 3.1.");
