...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53373");
  ...
  script_name("Debian: Security Advisory (DSA-306)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-306");
  script_xref(name:"URL", value:"https://www.debian.org/security/2003/dsa-306");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'ircii-pana' package(s) announced via the DSA-306 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Timo Sirainen discovered several problems in BitchX, a popular client for Internet Relay Chat (IRC). A malicious server could craft special reply strings, triggering the client to write beyond buffer boundaries or allocate a negative amount of memory. This could lead to a denial of service if the client only crashes, but may also lead to executing of arbitrary code under the user id of the chatting user.
...
We recommend that you upgrade your BitchX package.");
  ...
  script_tag(name:"affected", value:"'ircii-pana' package(s) on Debian 3.0.");
