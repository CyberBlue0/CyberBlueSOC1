...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53261");
  ...
  script_name("Debian: Security Advisory (DSA-568)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-568");
  script_xref(name:"URL", value:"https://www.debian.org/security/2004/dsa-568");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'cyrus-sasl-mit' package(s) announced via the DSA-568 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"A vulnerability has been discovered in the Cyrus implementation of the SASL library, the Simple Authentication and Security Layer, a method for adding authentication support to connection-based protocols. The library honors the environment variable SASL_PATH blindly, which allows a local user to link against a malicious library to run arbitrary code with the privileges of a setuid or setgid application.
...
We recommend that you upgrade your libsasl packages.");
  ...
  script_tag(name:"affected", value:"'cyrus-sasl-mit' package(s) on Debian 3.0.");
