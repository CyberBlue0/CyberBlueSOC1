...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53379");
  ...
  script_name("Debian: Security Advisory (DSA-399)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-399");
  script_xref(name:"URL", value:"https://www.debian.org/security/2003/dsa-399");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'epic4' package(s) announced via the DSA-399 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Jeremy Nelson discovered a remotely exploitable buffer overflow in EPIC4, a popular client for Internet Relay Chat (IRC). A malicious server could craft a reply which triggers the client to allocate a negative amount of memory. This could lead to a denial of service if the client only crashes, but may also lead to executing of arbitrary code under the user id of the chatting user.
...
We recommend that you upgrade your epic4 package.");
  ...
  script_tag(name:"affected", value:"'epic4' package(s) on Debian 3.0.");
