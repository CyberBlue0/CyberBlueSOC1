...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53418");
  ...
  script_name("Debian: Security Advisory (DSA-160)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-160");
  script_xref(name:"URL", value:"https://www.debian.org/security/2002/dsa-160");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'scrollkeeper' package(s) announced via the DSA-160 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Spybreak discovered a problem in scrollkeeper, a free electronic cataloging system for documentation. The scrollkeeper-get-cl program creates temporary files in an insecure manner in /tmp using guessable filenames. Since scrollkeeper is called automatically when a user logs into a Gnome session, an attacker with local access can easily create and overwrite files as another user.
...
We recommend that you upgrade your scrollkeeper packages immediately.");
  ...
  script_tag(name:"affected", value:"'scrollkeeper' package(s) on Debian 3.0.");
