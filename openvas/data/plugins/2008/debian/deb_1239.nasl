...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.57738");
  ...
  script_name("Debian: Security Advisory (DSA-1239)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-1239");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/dsa-1239");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'sql-ledger' package(s) announced via the DSA-1239 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Several remote vulnerabilities have been discovered in SQL Ledger, a web based double-entry accounting program, which may lead to the execution of arbitrary code. The Common Vulnerabilities and Exposures project identifies the following problems:
...
We recommend that you upgrade your sql-ledger packages.");
  ...
  script_tag(name:"affected", value:"'sql-ledger' package(s) on Debian 3.1.");
