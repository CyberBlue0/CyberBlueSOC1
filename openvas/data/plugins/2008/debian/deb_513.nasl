...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53204");
  ...
  script_name("Debian: Security Advisory (DSA-513)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-513");
  script_xref(name:"URL", value:"https://www.debian.org/security/2004/dsa-513");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'log2mail' package(s) announced via the DSA-513 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"jaguar@felinemenace.org discovered a format string vulnerability in log2mail, whereby a user able to log a specially crafted message to a logfile monitored by log2mail (for example, via syslog) could cause arbitrary code to be executed with the privileges of the log2mail process. By default, this process runs as user 'log2mail', which is a member of group 'adm' (which has access to read system logfiles).
...
We recommend that you update your log2mail package.");
  ...
  script_tag(name:"affected", value:"'log2mail' package(s) on Debian 3.0.");
