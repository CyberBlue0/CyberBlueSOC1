...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.53336");
  ...
  script_name("Debian: Security Advisory (DSA-265)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-265");
  script_xref(name:"URL", value:"https://www.debian.org/security/2003/dsa-265");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'bonsai' package(s) announced via the DSA-265 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Remi Perrot fixed several security related bugs in the bonsai, the Mozilla CVS query tool by web interface. Vulnerabilities include arbitrary code execution, cross-site scripting and access to configuration parameters. The Common Vulnerabilities and Exposures project identifies the following problems:
...
We recommend that you upgrade your bonsai package.");
  ...
  script_tag(name:"affected", value:"'bonsai' package(s) on Debian 3.0.");
