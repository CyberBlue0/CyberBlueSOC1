...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.55165");
  ...
  script_name("Debian: Security Advisory (DSA-784)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2008 Greenbone AG");
  script_family("Debian Local Security Checks");

  script_xref(name:"Advisory-ID", value:"DSA-784");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/dsa-784");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'courier' package(s) announced via the DSA-784 advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"A problem has been discovered in the Courier Mail Server. DNS failures were not handled properly when looking up Sender Policy Framework (SPF) records, which could allow attackers to cause memory corruption. The default configuration on Debian has SPF checking disabled, so most machines are not vulnerable. This is explained in the 'courier' manpage, section SENDER POLICY FRAMEWORK KEYWORDS.
...
We recommend that you upgrade your courier-mta package.");
  ...
  script_tag(name:"affected", value:"'courier' package(s) on Debian 3.1.");
