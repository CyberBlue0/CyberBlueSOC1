...
if(description)
{
  ...
  script_name("SolarWinds DameWare Mini Remote Control Detection (Windows SMB Login)");
  ...
  script_tag(name:"summary", value:"SMB login-based detection of SolarWinds DameWare Mini Remote
  Control.");

  script_xref(name:"URL", value:"https://www.solarwinds.com/dameware-mini-remote-control");
...
foreach key( key_list ) {
  foreach item( registry_enum_keys( key:key ) ) {
    ...
    # "DameWare Mini Remote Control" without any version
    appName = registry_get_sz( key:key + item, item:"DisplayName" );
    version = "unknown";
    location = "unknown";

    if( ! appName || appName !~ "DameWare Mini Remote Control" )
      continue;
    ...
    register_and_report_cpe( app:appName, ver:version, concluded:concluded,
                             base:"cpe:/a:solarwinds:dameware_mini_remote_control:", expr:"^([0-9.]+)",
