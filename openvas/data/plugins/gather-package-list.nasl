...
# @brief Cleans the output of dpkg-query and returns a package list for Notus Scanner.
#        This is done by dropping packages that are not actually installed (e.g. status "rc")
#        and always including an epoch in the version to make comparison more reliable.
#
# @param dpkg_query_buf The buffer containing the output of dpkg-query.
#                       The output must be in the format "${Package}##${Version}##${Status}##\n".
#
# @return A string, which contains one package per line.
#
function get_dpkg_packages( dpkg_query_buf ) {
  local_var dpkg_query_buf;
  local_var packages, lines, line, match, package;

  if ( ! dpkg_query_buf ) return;

  packages = "";
  lines = split( dpkg_query_buf, keep:FALSE );
  foreach line( lines ) {
    ...
    # Notus Scanner is more reliable when versions always contain an epoch. Add epoch 0, if there's not already an epoch.
    if( ":" >!< match[2] )
      package += "0:";
...
# nb:
# - No need to run this against WLC devices as they have a restricted shell and these devices are
#   already detected via gb_cisco_wlc_ssh_login_detect.nasl previously
# - No dependency to gb_cisco_wlc_ssh_login_detect.nasl has been added on purpose as some users
#   might only want to run a smaller scan config for e.g. authenticated SSH login-based package
#   checks and adding such a dependency would increase the scan time. The WLC detection should run
#   earlier then this VT and otherwise it shouldn't be also a big problem because there is an
#   additional check later to exit on WLC devices...
if( get_kb_item( "cisco/wlc/ssh-login/detected" ) )
  exit( 0 );
...
if( "-rbash: /bin/sh: restricted:" >< uname ) {
  set_kb_item( name:"ssh/restricted_shell", value:TRUE );
  replace_kb_item( name:"ssh/lsc/use_su", value:"no" );

  show_ver = ssh_cmd( socket:sock, cmd:"connectorctl version", nosh:TRUE, nosu:TRUE, return_errors:FALSE, pty:TRUE, clear_buffer:TRUE );

  if( "System Version" >< show_ver && "Service Manager Version" >< show_ver ) {
    set_kb_item( name:"ssh/login/cisco/spaces_connector/detected", value:TRUE );
    set_kb_item( name:"ssh/login/cisco/spaces_connector/port", value:port );
    set_kb_item( name:"ssh/login/cisco/spaces_connector/" + port + "/show_ver", value:show_ver );
    exit(0);
  }
}
...
if( _uname = egrep( string:uname, pattern:"^[ >]*(LANTIME V[0-9.]+|Meinberg (LANTIME|Funkuhren))", icase:FALSE ) ) {
  ...
  set_kb_item( name:"ssh/login/meinberg/lantime/detected", value:TRUE );
  set_kb_item( name:"ssh/login/meinberg/lantime/port", value:port );
  set_kb_item( name:"ssh/login/meinberg/lantime/" + port + "/full_login_banner", value:uname );
  set_kb_item( name:"ssh/login/meinberg/lantime/" + port + "/concluded", value:'    From login banner:\n' + chomp( _uname ) );
  exit( 0 );
}
...
# nb: Should be already covered via gb_cisco_wlc_ssh_login_detect.nasl but adding the KB keys here
# to make sure that these are set if a smaller scan config or similar was used and that VT wasn't
# running at all.
...
if( "Cyberoam Central Console" >< uname ) {
  ...
  if( ! isnull( ccc[1] ) ) {
    ...
    if( "CCC version:" >< version_info )
      set_kb_item( name:"sophos/cyberoam_central_console/version_info", value:version_info );
  }
  exit( 0 );
}
...
if( "NetMRI Administrative Shell" >< uname ) {
  set_kb_item( name:"ssh/no_linux_shell", value:TRUE );
  replace_kb_item( name:"ssh/lsc/use_su", value:"no" );
  set_kb_item( name:"ssh/force/pty", value:TRUE );

  set_kb_item( name:"ssh/login/infoblox/netmri/detected", value:TRUE );
  set_kb_item( name:"ssh/login/infoblox/netmri/port", value:port );

  cmd = "show version";

  show_vers = ssh_cmd( socket:sock, cmd:cmd, return_errors:FALSE, pty:TRUE, nosh:TRUE, nosu:TRUE,
                       clear_buffer:TRUE );
  set_kb_item( name:"ssh/login/infoblox/netmri/" + port + "/show_version", value:show_vers );
...
if( "NSX CLI" >< uname ) {
  get_ver = ssh_cmd( socket:sock, cmd:"get version", return_errors:FALSE, pty:TRUE, nosh:TRUE,
                     nosu:TRUE, timeout:20, retry:10, pattern:"VMware NSX Software", clear_buffer:TRUE );
  if( get_ver && "VMware NSX Software" >< get_ver ) {
    set_kb_item( name:"ssh/login/vmware/nsxt/detected", value:TRUE );
    set_kb_item( name:"ssh/login/vmware/nsxt/port", value:port );
    set_kb_item( name:"ssh/no_linux_shell", value:TRUE );
    replace_kb_item( name:"ssh/lsc/use_su", value:"no" );
    set_kb_item( name:"ssh/login/vmware/nsxt/" + port + "/get_version", value:get_ver );
    exit( 0 );
  }
}
...
if( "MXsecurity#" >< uname )
{
  set_kb_item( name:"ssh/no_linux_shell", value:TRUE );
  replace_kb_item( name:"ssh/lsc/use_su", value:"no" );
  set_kb_item( name:"ssh/login/moxa/mxsecurity/detected", value:TRUE );
  set_kb_item( name:"ssh/login/moxa/mxsecurity/port", value:port );
  exit( 0 );
}
...
if( "JUNOS" >< uname && "Junos Space" >!< uname )
{
  ...
  set_kb_item( name:"ssh/login/juniper/junos/port", value:port );
  exit( 0 );
}
...
if( "vRealize Network Insight Command Line Interface" >< uname )
{
  set_kb_item( name:"ssh/no_linux_shell", value:TRUE );
  set_kb_item( name:"ssh/force/pty", value:TRUE );
  replace_kb_item( name:"ssh/lsc/use_su", value:"no" );
  set_kb_item( name:"ssh/login/vmware/vrni/detected", value:TRUE );
  set_kb_item( name:"ssh/login/vmware/vrni/port", value:port );
  exit(0);
}
...
if( "% invalid command detected" >< uname || "syntax error: expecting" >< uname )
{
  ...
  if( "Cisco ACS VERSION INFORMATION" >< show_ver )
  {
    ...
    os_register_and_report( os:"Cisco Application Deployment Engine OS (ADE-OS)", cpe:ade_cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }

  if( "Cisco Identity Services Engine" >< show_ver )
  {
    set_kb_item( name:"ssh/login/cisco/ise/detected", value:TRUE );
    set_kb_item( name:"ssh/login/cisco/ise/port", value:port );
    set_kb_item( name:"ssh/login/cisco/ise/" + port + "/show_ver", value:show_ver );
    set_kb_item( name:"ssh/no_linux_shell", value:TRUE );
    replace_kb_item( name:"ssh/lsc/use_su", value:"no" );
...
if( "Unknown action 0" >< uname || "Command fail" >< uname ) {
...
if( "SECURECONNECTGATEWAY" >< uname ) {
  set_kb_item( name:"ssh/login/dell/secureconnectgateway/detected", value:TRUE );
  set_kb_item( name:"ssh/login/dell/secureconnectgateway/port", value:port );
  scg_version = ssh_cmd( socket:sock, cmd:"cat /etc/esrs-release", return_errors:FALSE);
  if( "SCG_VERSION" >< scg_version )
    set_kb_item( name:"ssh/login/dell/secureconnectgateway/" + port + "/version_info", value:scg_version );
  # nb: Secure Connect Gateway currently runs on SUSE Linux Enterprise Server so we don't exit here
}
...
if( ! is_pfsense ) {
  versinfo = ssh_cmd( socket:sock, cmd:"cat /etc/unraid-version", return_errors:FALSE );
  if( versinfo && versinfo =~ "^version\s*=" ) {
    set_kb_item( name:"ssh/login/unraid/detected", value:TRUE );
    set_kb_item( name:"ssh/login/unraid/port", value:port );
    set_kb_item( name:"ssh/login/unraid/" + port + "/vers_info", value:versinfo );
    exit( 0 );
  }
}
...
if( "VMware vRealize Log Insight" >< rls ) {
  set_kb_item( name:"ssh/login/vmware/vrealize_log_insight/detected", value:TRUE );
  set_kb_item( name:"ssh/login/vmware/vrealize_log_insight/port", value:port );
  set_kb_item( name:"ssh/login/vmware/vrealize_log_insight/" + port + "/rls", value:rls );
  set_kb_item( name:"ssh/login/vmware/vrealize_log_insight/" + port + "/rls_cmd", value:cmd );
  exit( 0 );
}

if( ! is_pfsense ) {
  cmd = "cat /etc/vmware/.buildInfo";
  rls = ssh_cmd( socket:sock, cmd:cmd, return_errors:FALSE );
...
if( "Space release " >< rls ) {
  ...
  set_kb_item( name:"ssh/login/juniper/junos/space/detected", value:TRUE );
  set_kb_item( name:"ssh/login/juniper/junos/space/port", value:port );
  set_kb_item( name:"ssh/login/juniper/junos/space/" + port + "/release", value:rls );
  exit( 0 );
}
...
if( ! is_pfsense ) {
  ...
  file = "/etc/issue";
  rls = ssh_cmd( socket:sock, cmd:"cat " + file, return_errors:FALSE );

  if( strlen( rls ) )
    _unknown_os_info += file + ': ' + rls + '\n\n';
}

# nb: See gsf/gb_meinberg_lantime_ssh_login_detect.nasl for an example "banner"
if( match = egrep( string:rls, pattern:"^\s*Meinberg LANTIME", icase:FALSE ) ) {

  set_kb_item( name:"ssh/login/meinberg/lantime/detected", value:TRUE );
  set_kb_item( name:"ssh/login/meinberg/lantime/port", value:port );
  set_kb_item( name:"ssh/login/meinberg/lantime/" + port + "/full_etc_issue", value:rls );
  set_kb_item( name:"ssh/login/meinberg/lantime/" + port + "/concluded", value:"    From file " + file + ':\n' + chomp( match ) );
  exit( 0 );
}
...
if( rls =~ "Barracuda CloudGen" ) {
  set_kb_item( name:"ssh/login/barracuda/cloudgen/detected", value:TRUE );
  set_kb_item( name:"ssh/login/barracuda/cloudgen/port", value:port );

  buf = ssh_cmd( socket:sock, cmd:"/opt/phion/bin/version.py", return_errors:FALSE );
  if( ! isnull( buf ) ) {
    set_kb_item( name:"ssh/login/barracuda/cloudgen/" + port + "/version_info", value:buf );
    exit( 0 );
  }
}
...
if( rls =~ "distrib_id=ubuntu" && rls =~ "distrib_release=" ) {
  ...
  if( vers[1] && vers[2] ) {
    ...
    if( buf ) {
      packages = get_dpkg_packages( dpkg_query_buf:buf );
      if( packages )
...
include( "gather-package-list-rockylinux.inc" );
_unknown_os_info = detect_rockylinux( sock:sock, port:port, SCRIPT_DESC:SCRIPT_DESC, is_pfsense:is_pfsense, _unknown_os_info:_unknown_os_info );
...
include( "gather-package-list-opensuse.inc" );
_unknown_os_info = detect_opensuse( sock:sock, port:port, SCRIPT_DESC:SCRIPT_DESC, is_pfsense:is_pfsense, _unknown_os_info:_unknown_os_info );
...
if( rls =~ "suse( linux)?" && rls !~ "enterprise" ) {
  ...
  vers = eregmatch( pattern:"suse (linux )?([0-9.]+)", string:rls, icase:TRUE );
  if( vers[2] ) {
    cpe += ":" + vers[2];
    oskey += vers[2];

    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os + " " + vers[2], rpm_access_error:error ) );
    os_register_and_report( os:os, version:vers[3], cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    os_register_and_report( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
...
include( "gather-package-list-archlinux.inc" );
_unknown_os_info = detect_archlinux( sock:sock, port:port, SCRIPT_DESC:SCRIPT_DESC, is_pfsense:is_pfsense, _unknown_os_info:_unknown_os_info );
...
if( rls == "Kerio OS" ) {
  set_kb_item( name:"ssh/login/kerio/control/detected", value:TRUE );
  set_kb_item( name:"ssh/login/kerio/control/port", value:port );

  version = ssh_cmd( socket:sock, cmd:"cat /etc/version", return_errors:FALSE );
  if (version)
    set_kb_item( name:"ssh/login/kerio/control/" + port + "/version", value:chomp( version ) );
  replace_kb_item( name:"ssh/lsc/use_us", value:"no" );
  exit( 0 );
}

include( "gather-package-list-openeuler.inc" );
_unknown_os_info = detect_openeuler( sock:sock, port:port, SCRIPT_DESC:SCRIPT_DESC, is_pfsense:is_pfsense, _unknown_os_info:_unknown_os_info );
