...
if (description)
{
  ...
  script_tag(name:"insight", value:"The jar_signature agent plugin (configured by the 'Signatures of
  certificates in JAR files' bakery rule) prepends JAVA_HOME/bin to the PATH environment variable.
  A user with write permission to that directory could replace legitimate commands with their own
  malicious scripts and execute them as root.");
