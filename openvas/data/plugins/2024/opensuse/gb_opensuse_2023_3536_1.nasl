# SPDX-FileCopyrightText: 2024 Greenbone AG
# Some text descriptions might be excerpted from (a) referenced
# source(s), and are Copyright (C) by the respective right holder(s).
#
# SPDX-License-Identifier: GPL-2.0-only

if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.833840");
  script_version("2025-02-26T05:38:41+0000");
  script_cve_id("CVE-2023-28840", "CVE-2023-28841", "CVE-2023-28842");
  script_tag(name:"cvss_base", value:"7.1");
  script_tag(name:"cvss_base_vector", value:"AV:N/AC:H/Au:N/C:N/I:C/A:C");
  script_tag(name:"last_modification", value:"2025-02-26 05:38:41 +0000 (Wed, 26 Feb 2025)");
  script_tag(name:"severity_vector", value:"CVSS:3.1/AV:N/AC:H/PR:N/UI:N/S:C/C:N/I:H/A:H");
  script_tag(name:"severity_origin", value:"NVD");
  script_tag(name:"severity_date", value:"2023-04-14 15:23:18 +0000 (Fri, 14 Apr 2023)");
  script_tag(name:"creation_date", value:"2024-03-04 07:30:47 +0000 (Mon, 04 Mar 2024)");
  script_name("openSUSE: Security Advisory for docker (SUSE-SU-2023:3536-1)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2024 Greenbone AG");
  script_family("SuSE Local Security Checks");

  script_xref(name:"Advisory-ID", value:"SUSE-SU-2023:3536-1");
  script_xref(name:"URL", value:"https://lists.opensuse.org/archives/list/security-announce@lists.opensuse.org/thread/QU3MTM4LJMUI2FUEWTJTE2U7YEPFOJLG");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'docker'
  package(s) announced via the SUSE-SU-2023:3536-1 advisory.
Note: This VT has been deprecated and replaced by a Notus scanner based one.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"This update for docker fixes the following issues:

  * Update to Docker 24.0.5-ce.

  * Update to Docker 24.0.4-ce.


  * Update to Docker 24.0.3-ce.


  * Recommend docker-rootless-extras instead of Require(ing) it, given it's an
      additional functionality and not inherently required for docker to function.


  * Update to Docker 24.0.2-ce. See upstream changelog online at

  * Add Recommends for docker-buildx, and add /usr/lib/docker/cli-plugins as
      being provided by this package.

  * was rebuilt against current GO compiler.");

  script_tag(name:"affected", value:"'docker' package(s) on openSUSE Leap 15.4, openSUSE Leap 15.5.");

  script_tag(name:"solution", value:"Please install the updated package(s).");

  script_tag(name:"solution_type", value:"VendorFix");
  script_tag(name:"qod_type", value:"package");

  script_tag(name:"deprecated", value:TRUE);

  exit(0);
}

exit(66);
